...
import khorunzhyicom.alex.theposter.api.actions.common.AuthorizableAction;
...
public class PosterHttpService extends PosterActionServiceWrapper {
    ...
    protected <A> boolean onInterceptSend(ActionHolder<A> actionHolder) throws JanetException {
        if (actionHolder.action() instanceof AuthorizableAction) {
            AuthorizableAction action = (AuthorizableAction) actionHolder.action();
            authorizeAction(action);
        }
        return false;
    }

    private void authorizeAction(AuthorizableAction action) {
